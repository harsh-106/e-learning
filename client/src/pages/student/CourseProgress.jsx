...
const CourseProgress = () => {
  ...
  if (isError) return <p>Fail to load course detail</p>
  console.log(data);
  ...
  //initialize if first lecture is not exist
  ...
  return (
    <div className='max-w-7xl mx-auto p-4 '>
      ...
      <div className='flex justify-between mb-4'>
        ...
        <Button variant={completed ? "outline" : "default"}  onClick={completed ? handleInCompleteCourse : handleCompleteCourse} >{completed ? <div className='flex items-center' > <CheckCircle className='h-4 w-4 mr-2' /> <span>Completed</span> </div> : "Mark as completed" }</Button>
      </div>
        ...
        <div className='flex-1 md:w-3/5 h-fit rounded-lg shadow-lg p-4'>
          ...
          <div className='mt-2'>
            <h3 className='font-medium text-lg'>
              {
                `Lecure ${courseDetails.lectures.findIndex((lec) => lec._id === (currentLecture?._id || initialLecture._id)) + 1} : ${currentLecture?.lectureTitle || initialLecture.lectureTitle}`
              }
            </h3>
          </div>
        </div>
          ...
          <div className='flex-1 overflow-y-auto'>
            {
              courseDetails?.lectures.map((lecture) => (
                <Card key={lecture._id} onClick={() => handleSelectLecture(lecture)} className={`mb-3 hover:cursor-pointer transition transform ${lecture._id === currentLecture?._id ? 'bg-gray-200 dark:dark:bg-gray-800' : ''}` }>
